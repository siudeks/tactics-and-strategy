...
- REQ-ORD-MOVE-002: Movement phase executes during turn simulation and consumes pending `MOVE` orders assigned in the command phase. Each order is validated at execution time (not at assignment time): the target coordinates must lie within map bounds and the scenario's default terrain must be traversable (not `VOID` or `WATER`). Invalid targets are silently dropped — no exception is raised, no partial move occurs, the order is not surfaced as cancelled to the caller, and the unit remains in place. During stepwise runtime execution the movement step updates the in-session state first and emits `MovementPlayback` for UI choreography before final end-turn commit. Implementation: `SimultaneousMovePhaseExecutor` consumes `CampaignState.pendingOrders`.
- REQ-MOVE-002: The movement phase resolves terrain-cost-aware orthogonal routes with deterministic tie-breaking. Step cost is derived from scenario default terrain (`SAND=1`, `MOUNTAIN=3`), and equal-cost alternatives are broken deterministically by `(totalCost, y, x)` ordering so repeated runs over identical inputs choose the same route.
...
- REQ-RTS-MOVE-001: When a MOVE target is assigned to a unit, the unit begins moving continuously toward that target at a speed of 1 tile per in-game hour (= 1 tile per real second). Movement is visible on the map immediately without waiting for END_TURN. The unit's tile coordinate in campaign state is updated atomically when it arrives at the target.
- REQ-RTS-MOVE-002: While the game clock is paused (SPACE), all real-time unit movement is also paused. Movement resumes when the clock is unpaused.
- REQ-RTS-MOVE-003: If END_TURN is triggered before a unit reaches its assigned target, the `SIMULTANEOUS_MOVE` phase finalizes the unit at its assigned target (existing turn-engine behavior). The RTS movement tracker is cleared after END_TURN completes so the next command phase starts with no in-progress movements.
