...
- REQ-UI-PHASE-001: After End Turn simulation advances runtime state, the UI shall present a fullscreen dimmed phase overlay sequence sourced from `TurnResult.phaseTrace`; each phase (including `RETREAT`) shall remain visible for a fixed 3.0 seconds using UI-only timing (no engine-side delay insertion).
- REQ-UI-LOCK-001: While the phase overlay sequence is active, map interaction paths and related key shortcuts (click, drag, zoom, selection, key shortcuts) shall be locked, and the HUD lock state shall disable HUD actions until the sequence completes. End Turn requests during the active sequence shall be idempotent (no additional turn simulation) and become accepted again only after overlay completion.
