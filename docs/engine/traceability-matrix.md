...
| REQ-UI-MOVE-002 | Clicking valid destination replaces preview with persistent small flag marker and exits selection state | Planned | None yet (to add confirmation-flow and render-state tests) | Confirmation must end preview state, ignore clicks without valid preview, and leave a stable marker on the assigned hex. |
